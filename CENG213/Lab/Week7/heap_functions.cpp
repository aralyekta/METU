...
int kthsmallest(HeapPriorityQueue<int> heap, int k) {
  int removed_val, return_val;
  std::queue<int> restoreQueue;
  if (k <= 0)
  {
  ...
  {
    for (int i = 0; !heap.isEmpty() && i < k-1; i++)
    {
      removed_val = heap.remove();
      restoreQueue.push(removed_val);
    }
    if (heap.isEmpty())
    {
      while (!restoreQueue.empty())
      {
        removed_val = restoreQueue.front();
        heap.add(removed_val);
        restoreQueue.pop();
      }
      throw IllegalArgumentException();
    }
...
bool isConsecutive(HeapPriorityQueue<int> heap) {
  int prev_val, restore_val;
  std::queue<int> restoreQueue;
  if (!heap.isEmpty())
  {
    prev_val = heap.remove();
    restoreQueue.push(prev_val);
  }
  while (!heap.isEmpty())
  {
    if (prev_val - heap.peek() == -1)
    {
      prev_val = heap.remove();
      restoreQueue.push(prev_val);
    }
    else
    {
      while (!restoreQueue.empty())
      {
        restore_val = restoreQueue.front();
        heap.add(restore_val);
        restoreQueue.pop();
      }
      return false;
    }
